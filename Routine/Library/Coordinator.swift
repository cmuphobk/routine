...
extension Coordinatorable {
    ...
    func viewControllerDidBackForward(_ viewController: UIViewController) {
        for coordinator in self.childCoordinators {
            if let index = coordinator.childViewControllers.index(ref: viewController) {
                coordinator.childViewControllers.remove(at: index)
                if coordinator.childViewControllers.count == 0 {
                    coordinator.parentCoordinator?.childNeedFinish(self)
                }
                return
            }
            coordinator.viewControllerDidBackForward(viewController)
        }
    }
